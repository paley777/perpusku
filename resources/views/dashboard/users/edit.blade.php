...
@section('container')
    ...
    <h2 class="mt-3 mb-4">Edit Petugas</h2>
    <span>Apabila data tidak tersedia, masukan strip ( - ) untuk setiap kolom.</span>

    <form method="post" action="/dashboard/users/{{ $user->id }}" class="row mt-3 g-3">
        @method('put')
        @csrf
        <div class="col-md-4">
            <label for="inputEmail3" class="form-label">ID</label>
            <input type="text" class="form-control" name="id" value="{{ old('id', $user->id) }}" readonly>
        </div>
        <div class="col-md-4">
        ...
        <div class="col-md-4">
            ...
            <input type="text" class="form-control" placeholder="Contoh : XXXXXX" name="nip"
                value="{{ old('nip', $user->nip) }}" required>
        </div>
        <div class="col-md-4">
            <label for="inputEmail3" class="form-label">Tanggal Lahir</label>
            <input type="date" class="form-control" name="tgl_lahir" value="{{ old('tgl_lahir', $user->tgl_lahir) }}">
        </div>
        <div class="col-md-4">
            <label for="inputEmail3" class="form-label">Alamat</label>
            <input type="text" class="form-control" placeholder="Contoh : JL. Merdeka 12" name="alamat"
                value="{{ old('alamat', $user->alamat) }}" required>
        </div>
        <div class="col-md-4">
            <label for="inputEmail3" class="form-label">Nomor Telepon</label>
            <input type="text" class="form-control" placeholder="Contoh : 08XX-XXXX-XXXX" name="no_tlp"
                value="{{ old('no_tlp', $user->no_tlp) }}" required>
        </div>
        <div class="col-md-4">
            <label for="inputEmail3" class="form-label">E-Mail</label>
            <input type="text" class="form-control" placeholder="Contoh : XXXXXX@gmail.com" name="email"
                value="{{ old('email', $user->email) }}" required>
        </div>
        <div class="col-md-4">
            <label for="inputEmail3" class="form-label">Jabatan</label>
            <input type="text" class="form-control" placeholder="Contoh : Administrator" name="jabatan"
                value="{{ old('jabatan', $user->jabatan) }}" required>
        </div>
        <div class="col-md-4">
            <label for="inputEmail3" class="form-label">Username</label>
            <input type="text" class="form-control" placeholder="Contoh : admin123" name="username"
