...
@section('container')
    ...
    <form method="post" action="/dashboard/kembalis/{{ $kembali->id }}" class="row mt-3 g-3">
        @method('put')
        @csrf
        <div class="col-md-4">
            <label for="inputEmail3" class="form-label">ID</label>
            <input type="text" class="form-control" name="id" value="{{ old('id', $kembali->id) }}" placeholder=""
                required readonly>
        </div>
        <div class="col-md-4">
            <label for="inputEmail3" class="form-label">Tanggal Transaksi</label>
            <input type="date" class="form-control" name="tgl" value="{{ old('tgl', $kembali->tgl) }}">
        </div>
        <div class="col-md-4">
            <label for="inputEmail3" class="form-label">Nama Lengkap</label>
            <input type="text" class="form-control" name="nama" value="{{ old('nama', $kembali->nama) }}"
                placeholder="Masukkan Nama Siswa." autofocus required>
        </div>
        <div class="col-md-4">
            <label for="inputEmail3" class="form-label">Kelas</label>
            <input type="text" class="form-control" name="kelas" value="{{ old('kelas', $kembali->kelas) }}"
                placeholder="Masukkan Kelas Siswa." required>
        </div>
        <div class="col">
            <label for="inputEmail3" class="form-label">Buku yang Dikembalikan</label>
            <input type="text" class="form-control" name="kembali" value="{{ old('kembali', $kembali->kembali) }}"
